#------------------------------------------------------------------------------
# Prusa Slicer JPEG Thumbnail creator
...
from prusa_slicer.utils import console
from prusa_slicer.SliceLayer import SliceLayer
from prusa_slicer.PrintObject import PrintObject
from prusa_slicer.settings import Settings
...
class CreateKobraThumbnail(Script):
    ...
    def _createSnapshot(self):
        ...
        try:
            ...
            console.log_exception("Failed to create snapshot image")

    def _encodeSnapshot(self, snapshot):
        try:
            thumbnail_buffer = BytesIO()
            thumbnail_image = snapshot
            thumbnail_image.save(thumbnail_buffer, "JPEG", quality=50)
            hex_bytes = binascii.hexlify(thumbnail_buffer.getvalue())
            hex_message = hex_bytes.decode('ascii')
            thumbnail_buffer.close()
            return hex_message
        except Exception:
            console.log_exception("Failed to encode snapshot image")
	...
	def execute(self, data: PrintObject):
		...
		if snapshot:
			...
			for layer in data.layers:
				...
				for line in lines:
					if line.startswith(";Generated with PrusaSlicer"):
				...
				final_lines = "\n".join(lines)
				layer.layer_content = final_lines
